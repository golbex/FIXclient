...
func QueryAction() (string, error) {
	...
	fmt.Println("3) Request Order Status")
	fmt.Println("4) Quit")
	fmt.Print("Action: ")
	scanner := bufio.NewScanner(os.Stdin)
	scanner.Scan()
...
func queryOrderQty() field.OrderQtyField {
	return field.NewOrderQty(queryDecimal("OrderSize"), 2)
}
...
func QueryStatusOrder() (err error) {
	defer func() {
		if e := recover(); e != nil {
			err = e.(error)
		}
...
func queryOrderStatusRequest42() (msg *quickfix.Message) {
	status := fix42st.New(queryClOrdID(),querySymbol(),querySide())
	msg = status.ToMessage()
	queryHeader(&msg.Header)
	return
}